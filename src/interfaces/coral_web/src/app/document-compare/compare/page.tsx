...
const documents = [
  { id: 'doc-a', title: 'Agreement A', meta: 'PDF · 42 pages · Uploaded 2m ago', status: 'Ready' },
  { id: 'doc-b', title: 'Agreement B', meta: 'PDF · 37 pages · Uploaded 5m ago', status: 'Ready' },
  { id: 'doc-c', title: 'Annex C', meta: 'PDF · 18 pages · Missing upload', status: 'Missing' },
];
        ...
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.4fr_1fr]">
            <div className="flex flex-col gap-4 rounded-2xl border border-marble-400 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-label text-volcanic-700">Document set</p>
                  <h2 className="text-h5 font-variable text-volcanic-900">Select files</h2>
                </div>
                <span className="rounded-full bg-blue-100 px-3 py-1 text-label text-blue-700">2 selected</span>
            </div>
              ...
              {documents.map((doc, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    doc.status === 'Missing' ? 'border-secondary-200 bg-secondary-50' : 'border-blue-100 bg-blue-50'
                  )}
                >
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <span
                      className={cn(
                        ...
                        doc.status === 'Missing' ? 'bg-secondary-200 text-volcanic-600' : 'bg-blue-100 text-blue-700'
                      )}
                    >
                      {doc.status}
                    </span>
                  ...
                  <button
                    className={cn(
                      ...
                      index < 2 ? 'border-blue-200 text-blue-700' : 'border-secondary-200 text-volcanic-600'
                    )}
                  >
            ...
            <div className="rounded-xl border border-marble-300 bg-white p-3">
              ...
              <div className="mt-3 grid gap-2">
                {['Clause diff', 'Table alignment', 'Image similarity'].map((mode) => (
                  <div key={mode} className="flex items-center justify-between rounded-lg bg-secondary-50 px-3 py-2">
                    ...
                    <span className="text-label text-blue-700">On</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-4 rounded-2xl border border-marble-400 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-label text-volcanic-700">Inline diff</p>
                    <h2 className="text-h5 font-variable text-volcanic-900">Clause comparison</h2>
                  </div>
              <span className="rounded-full bg-blue-100 px-3 py-1 text-label text-blue-700">Doc A vs Doc B</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {changeBadges.map((badge) => (
                <span key={badge.label} className={`rounded-full px-3 py-1 text-label ${badge.className}`}>
                  {badge.label}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-marble-400 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <Link href="/document-compare/deep-dive" className="rounded-full border border-blue-200 px-3 py-1 text-p-sm text-blue-700">
                Open deep dive
              </Link>
            </div>
            <div className="space-y-3">
              {evidenceItems.map((item) => (
                <div key={item.title} className="rounded-xl border border-marble-300 bg-secondary-50 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-p text-volcanic-800">{item.title}</p>
                      <p className="text-p-xs text-volcanic-600">{item.type}</p>
                    </div>
                    <span className="rounded-full bg-blue-100 px-2 py-1 text-label text-blue-700">{item.change}</span>
                  </div>
                  ...
                  {item.type === 'Table' ? (
                    <div className="mt-3 overflow-hidden rounded-lg border border-marble-300 bg-white">
                      ...
                      {udsRows.map((row) => (
                        <div key={`${row.service}-${row.id}`} className="grid grid-cols-[0.8fr_1.2fr_0.6fr_0.7fr_0.7fr_0.6fr] border-t border-marble-300 text-p-xs text-volcanic-700">
                          ...
                          <div className="px-3 py-2">
                            <span className="rounded-full bg-blue-100 px-2 py-1 text-label text-blue-700">{row.change}</span>
                          </div>
                        </div>
                      ))}
                    </div>
