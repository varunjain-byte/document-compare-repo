...
const quickLinks = [
  {
    title: 'Upload',
    description: 'Create a comparison set and upload the PDFs to analyze.',
    href: '/document-compare/upload',
    accent: 'bg-blue-300',
  },
  {
    title: 'Parse PDFs',
    description: 'Upload and segment documents into zones, tables, and figures.',
    href: '/document-compare/parse',
    accent: 'bg-blue-500',
  },
  {
    title: 'Structured Format',
    description: 'View the normalized outline, clauses, and table schema.',
    href: '/document-compare/structure',
    accent: 'bg-blue-700',
  },
  {
    title: 'Compare',
    description: 'Align tables, images, and text across two or more PDFs.',
    href: '/document-compare/compare',
    accent: 'bg-blue-900',
  },
];
        ...
        <aside className="flex h-full flex-col gap-6 px-4 py-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-p-sm text-volcanic-600">
              Parse PDFs, structure them into a clean schema, then compare content side by side.
            </p>
          </div>
          <div className="rounded-2xl border border-marble-400 bg-white p-4">
            ...
            <div className="mt-3 space-y-2">
              <div className="flex items-center justify-between rounded-lg bg-secondary-50 px-3 py-2">
                <span className="text-p-sm text-volcanic-800">ZF Procurement</span>
                <span className="text-label text-blue-700">Active</span>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-secondary-50 px-3 py-2">
                <span className="text-p-sm text-volcanic-800">R&D Compliance</span>
                <span className="text-label text-volcanic-600">Review</span>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-secondary-50 px-3 py-2">
                <span className="text-p-sm text-volcanic-800">Legal Ops</span>
                <span className="text-label text-volcanic-600">Paused</span>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-marble-400 bg-white p-4">
            ...
            <header className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full bg-blue-700 px-3 py-1 text-label text-white">
                    ZF Theme
                  </span>
                ...
                <h1 className="text-h3 font-variable text-volcanic-900">
                  Blue-first document portal for parsing, structuring, and comparing PDFs.
                </h1>
                <p className="text-p-lg text-volcanic-700">
                  Upload document sets, inspect structured output, then compare tables, images, and
                  text with a clear ZF-style blue interface.
                </p>
                <div className="flex flex-wrap gap-3">
                  <button className="rounded-full bg-blue-700 px-5 py-2 text-p text-white shadow-md transition hover:bg-blue-900">
                    Upload PDFs
                  </button>
                  <button className="rounded-full border border-blue-200 bg-white px-5 py-2 text-p text-blue-700 transition hover:border-blue-500">
                    Connect SharePoint
                  </button>
                </div>
              </div>
            ...
            <section className="grid gap-5 lg:grid-cols-3">
              {quickLinks.map((item) => (
                <Link
                  key={item.title}
                  href={item.href}
                  className="group rounded-2xl border border-marble-400 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-300"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-h5 font-variable text-volcanic-900">{item.title}</span>
                    <span
                      className={cn(
                        'h-10 w-10 rounded-full text-white shadow-sm transition',
            ...
            <section className="rounded-2xl border border-marble-400 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="text-h4 font-variable text-volcanic-900">
                    Document comparison pipeline
                  </h2>
                </div>
                <Link
                  href="/document-compare/compare"
                  className="rounded-full bg-blue-700 px-4 py-2 text-p-sm text-white"
                >
                  Go to comparison
                </Link>
              </div>
