  ...
  const leftDrawerElement = (
    <aside className="flex h-full flex-col gap-6 px-4 py-6 text-volcanic-900">
      ...
      <div className="space-y-2">
        ...
        <p className="text-p-sm text-volcanic-600">Add PDFs to a named set before parsing and comparing.</p>
      </div>
      ...
      <div className="relative flex flex-col gap-8 p-6 lg:p-10">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-h3 font-variable text-volcanic-900">Create a comparison batch</h1>
            <p className="text-p text-volcanic-700">Name the batch and upload the PDFs you want to compare.</p>
          </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-marble-400 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {['Batch ID: ZF-2024-048', 'Owner: Procurement', 'Region: EMEA', 'Due: 12 Mar'].map((item) => (
                <div key={item} className="rounded-xl border border-marble-300 bg-secondary-50 px-4 py-3 text-p-sm text-volcanic-700">
                  {item}
                </div>
              ))}
            </div>
            <div className="mt-5 flex flex-wrap gap-3">
              <button className="rounded-full bg-blue-700 px-4 py-2 text-p-sm text-white">Upload PDFs</button>
              <button className="rounded-full border border-blue-200 px-4 py-2 text-p-sm text-blue-700">Connect SharePoint</button>
              <button className="rounded-full border border-blue-200 px-4 py-2 text-p-sm text-blue-700">Import from Drive</button>
            </div>
          </div>
          <div className="rounded-2xl border border-marble-400 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {uploadCards.map((card) => (
                <div key={card.title} className="flex items-center justify-between rounded-xl border border-marble-300 bg-secondary-50 px-4 py-3">
                  <div>
                    <p className="text-p text-volcanic-800">{card.title}</p>
                    <p className="text-p-xs text-volcanic-600">{card.detail}</p>
                  </div>
                  <span className="rounded-full bg-blue-50 px-3 py-1 text-label text-blue-700">{card.status}</span>
                </div>
              ))}
            </div>
          </div>
